...
import { mockApiKeys, mockWebhooks, mockIntegrations, apiEndpoints, availableWebhookEvents } from "@/mocks/data/api-keys";
...
export default function ApiKeysPage() {
  ...
  const handleGenerateKey = async (name: string, permissions: string[]): Promise<string> => {
    // Simulate API call
    await new Promise((resolve) => setTimeout(resolve, 1000));

    const fullKey = `asha_sk_${Math.random().toString(36).substring(2, 15)}${Math.random().toString(36).substring(2, 15)}`;
    const newKey: ApiKey = {
      id: `key${Date.now()}`,
      name,
      keyPrefix: `asha_sk_***...${fullKey.slice(-3)}`,
      permissions,
  ...
  const handleCopyBaseUrl = () => {
    navigator.clipboard.writeText("https://api.asha.id/v1");
    toast.success("Copied", {
      description: "Base URL copied to clipboard",
    });
  };

  const handleAddWebhook = (webhook: Omit<Webhook, "id" | "createdAt" | "secret" | "failureCount">) => {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">API Keys & Integrations</h1>
        <p className="text-slate-500 dark:text-slate-400">Manage API access and third-party integrations</p>
      </div>

      {/* API Keys */}
      <ApiKeysTable keys={apiKeys} onGenerate={handleGenerateKey} onRevoke={handleRevokeKey} onCopy={handleCopyKey} />

      {/* API Documentation */}
      <ApiDocumentation baseUrl="https://api.asha.id/v1" endpoints={apiEndpoints} onCopyBaseUrl={handleCopyBaseUrl} />
